...
export default async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    if (req.body && req.body.checkForDuplicateReport) {
      return handleCheckForDuplicateReport(req, res, session.org);
    }

    if (req.body && req.body.createStaffReportedIssue) {
      return handleCreateStaffReportedIssue(req, res, session.u, session.org);
    }
    ...
    if (req.body && req.body.editUnit) {
      ...
      try {
        ...
        if (rentAmount !== undefined) {
          const newRent = rentAmount === "" || rentAmount === null ? null : Number(rentAmount);
          if (newRent !== (before.rent_amount_tzs !== null ? Number(before.rent_amount_tzs) : null)) {
            fields.rent_amount_tzs = newRent;
            changes.push(`Rent Amount: "${before.rent_amount_tzs || "not set"}" → "${newRent || "not set"}"`);
          }
        }
        if (serviceChargeAmount !== undefined) {
          const newCharge = serviceChargeAmount === "" || serviceChargeAmount === null ? null : Number(serviceChargeAmount);
          if (newCharge !== (before.service_charge_amount_tzs !== null ? Number(before.service_charge_amount_tzs) : null)) {
            fields.service_charge_amount_tzs = newCharge;
            changes.push(`Service Charge: "${before.service_charge_amount_tzs || "not set"}" → "${newCharge || "not set"}"`);
          }
        }
    ...
    // two is actually set (a unit could reasonably have only one) —
    // rejects outright if neither is set, since an invoice for an
    // unspecified amount isn't a real invoice. Deliberately manual for
    // now rather than tied to the existing bi-annual notice cron — a
    // simpler, more inspectable first version; auto-generation on the
    // same schedule as the reminder is a natural next step once this
    // foundation is proven, not assumed here.
    if (req.body && req.body.generateInvoice) {
      if (session.r === "technician") {
        return res.status(403).json({ error: "Not permitted to generate an invoice." });
      }
      const { unitId, periodStart, periodEnd, dueDate } = req.body;
      if (!unitId || !periodStart || !periodEnd || !dueDate) {
        return res.status(400).json({ error: "unitId, periodStart, periodEnd, and dueDate are required" });
      }
      try {
        const { getById, insert } = await import("../lib/postgresClient.js");
        const unit = await getById("units", unitId).catch(() => null);
        if (!unit) return res.status(404).json({ error: "Unit not found" });

        const rent = unit.rent_amount_tzs !== null ? Number(unit.rent_amount_tzs) : 0;
        const serviceCharge = unit.service_charge_amount_tzs !== null ? Number(unit.service_charge_amount_tzs) : 0;
        const amount = rent + serviceCharge;
        if (amount <= 0) {
          return res.status(400).json({ error: "This unit has no rent or service charge amount set yet — add one under Edit before generating an invoice." });
        }

        const invoice = await insert("unit_invoices", {
          unit_id: unitId, period_start: periodStart, period_end: periodEnd,
          amount_tzs: amount, due_date: dueDate, status: "Unpaid", generated_by: session.u,
        }, { typecast: true });

        await appendUnitActivityLog(unitId, `🧾 Invoice generated by ${session.u} — TZS ${amount.toLocaleString()} for ${periodStart} to ${periodEnd}, due ${dueDate}`, session.u, "system");
    ...
    // Recording that a payment happened — the deliberate boundary
    // confirmed before building any of this: FAM records the fact of
    // a payment, it never moves money itself. invoiceId is optional -
    // a payment can be recorded before anyone's had a chance to
    // allocate it to a specific invoice. When it IS tied to an
    // invoice, that invoice's status is recomputed from the sum of
    // every payment actually applied to it, not just this one, so
    // partial payments across multiple entries add up correctly.
    if (req.body && req.body.recordPayment) {
      if (session.r === "technician") {
        return res.status(403).json({ error: "Not permitted to record a payment." });
      }
      const { unitId, invoiceId, amount, paymentDate, paymentMethod, paymentReference, notes } = req.body;
      if (!unitId || !amount || Number(amount) <= 0 || !paymentDate || !paymentMethod) {
        return res.status(400).json({ error: "unitId, amount, paymentDate, and paymentMethod are required" });
      }
      try {
        const { insert, getById, update, query: pgQuery } = await import("../lib/postgresClient.js");

        const payment = await insert("unit_payments", {
          unit_id: unitId, invoice_id: invoiceId || null, amount_tzs: Number(amount),
          payment_date: paymentDate, payment_method: paymentMethod,
          payment_reference: (paymentReference || "").trim() || null,
          notes: (notes || "").trim() || null, recorded_by: session.u,
        }, { typecast: true });

        if (invoiceId) {
          const invoice = await getById("unit_invoices", invoiceId).catch(() => null);
          if (invoice) {
            const paymentsResult = await pgQuery("select coalesce(sum(amount_tzs), 0) as total from unit_payments where invoice_id = $1", [invoiceId]);
            const totalPaid = Number(paymentsResult.rows[0].total);
            const invoiceAmount = Number(invoice.amount_tzs);
            const newStatus = totalPaid >= invoiceAmount ? "Paid" : totalPaid > 0 ? "Partially Paid" : "Unpaid";
            await update("unit_invoices", invoiceId, { status: newStatus });
          }
        }

        await appendUnitActivityLog(unitId, `💵 Payment recorded by ${session.u} — TZS ${Number(amount).toLocaleString()} via ${paymentMethod}${paymentReference ? ` (ref: ${paymentReference})` : ""}`, session.u, "system");
        return res.status(200).json({ success: true, paymentId: payment.id });
      } catch (err) {
...
// Same category -> role mapping already used on the public no-login
// report links, kept as a local copy rather than a cross-file import -
// each api/*.js file here is its own independent function bundle,
// confirmed by how every other file in this codebase already works.
const STAFF_REPORT_CATEGORY_TO_ROLE = {
...
// Live duplicate check for the new staff "Report an Issue" screen -
// called BEFORE the person submits, so they can see a real warning
// and still choose to submit anyway. Confirmed directly: reporters
// describe symptoms and location, not asset IDs, so this compares
// real language against a real, narrow candidate pool (same building,
// still Open, reported recently) rather than any kind of exact match.
async function handleCheckForDuplicateReport(req, res, organizationId) {
  const { description, building } = req.body || {};
  if (!description || !description.trim() || !building) {
    return res.status(200).json({ match: null }); // nothing to compare yet - not an error, just too early to check
  }
  try {
    const { query: pgQuery } = await import("../lib/postgresClient.js");
    const { findLikelyDuplicate } = await import("../lib/duplicateReportAI.js");
    const match = await findLikelyDuplicate({ pgQuery, description: description.trim(), building, organizationId });
    return res.status(200).json({ match });
  } catch (err) {
    console.error("handleCheckForDuplicateReport error (non-fatal):", err.message);
    return res.status(200).json({ match: null }); // a failed check should never block someone from reporting a real issue
  }
}

// The actual staff-facing "Report an Issue" screen - the first real
// way a logged-in staff member can log a general issue themselves,
// confirmed directly: no such screen existed before this. Reporter is
// the real session user, not an anonymous portal visitor. Runs the
// same AI urgency assessment as the public report paths (this is also
// a human report with real language to interpret, no asset tied to
// it), and reuses the SAME duplicate-check logic for a final record
// on the new work order - the live warning already shown before
// submit doesn't need repeating, this is just consistent bookkeeping.
async function handleCreateStaffReportedIssue(req, res, reportedByUsername, organizationId) {
  const { description, facility, building, floor, roomZone, category } = req.body || {};
  if (!description || !description.trim() || !building || !floor || !category) {
    return res.status(400).json({ error: "A building, floor, category, and description are all required." });
  }
  const assignedRole = STAFF_REPORT_CATEGORY_TO_ROLE[category] || "Admin";
  const combinedBuilding = facility && facility.trim() && facility.trim() !== building.trim() ? `${facility.trim()} — ${building.trim()}` : building.trim();
  ...
  try {
    ...
    const recentOrgHistory = (recentHistoryResult?.rows || []).map(r => ({ summary: (r.notes || "").slice(0, 100), urgency: r.urgency }));
    const urgencyAssessment = await assessWorkOrderUrgency({
      reportText: `${description} (reported at ${location})`,
      asset: null,
      multiIssueLikely: false,
      isLeadershipReporter: false, // this reporter has a real logged-in role already - the Leadership Reporter floor is specifically about an unauthenticated/email reporter's seniority being otherwise invisible to the system, not relevant here
      recentOrgHistory,
    });
    ...
    const created = await insert("work_orders", {
      ...
      notes: `Reported by ${reportedByUsername} at ${location}: ${description}`,
      reporter_contact: reportedByUsername,
    ...
    // Same retroactive record kept for consistency, even though the
    // live warning already ran before this submission - this just
    // makes sure the saved record itself carries the same flag
    // anyone browsing the list later would also want to see.
    try {
      const { findLikelyDuplicate } = await import("../lib/duplicateReportAI.js");
      const match = await findLikelyDuplicate({ pgQuery, description: description.trim(), building: combinedBuilding, organizationId, excludeRecordId: created.id });
      if (match) {
        openingLog.push({ text: `⚠ Possibly the same issue as ${match.woId}${match.reason ? ` — ${match.reason}` : ""}`, by: "system", at: new Date().toISOString() });
        await update("work_orders", created.id, { possible_duplicate_of: match.woId }).catch(() => {});
      }
    } catch (err) {
      console.error("Duplicate check failed on staff report (non-fatal):", err.message);
    }

    await update("work_orders", created.id, { activity_log: JSON.stringify(openingLog) }).catch(() => {});

    return res.status(200).json({ success: true, woId, recordId: created.id });
  } catch (err) {
    console.error("handleCreateStaffReportedIssue error:", err);
    return res.status(500).json({ error: err.message });
  }
}
...
async function handleMaintenanceReport(req, res, organizationId) {
  ...
  try {
    ...
    const workOrders = await Promise.all(filtered.map(async r => {
      ...
      completedDate: r.completed_date || "", closedBy: r.closed_by || "", closureSummary: r.closure_summary || "", possibleDuplicateOf: r.possible_duplicate_of || "",
      cost: r.cost_tzs !== null ? Number(r.cost_tzs) : null,
