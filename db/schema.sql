...
-- ============================================================
-- Real, explicitly-defined zones - confirmed directly: the real
-- hierarchy is Facility < Building < Floor < Zone < Room, with a
-- zone nested under a specific floor, and genuinely optional - a
-- floor doesn't need one at all. Free text, not a coded convention
-- like floor_id, matching how components.room_zone already works.
-- ============================================================
...
-- ============================================================
-- Opaque login tokens, requested directly, even for Master System's
-- own login - previously a client's link used a readable, name-
-- derived slug, and Master System itself had no slug at all,
-- resolved instead by a special "no slug provided" default. Both
-- replaced: client slugs are now random, unguessable tokens
-- (generated in code, see api/login.js), and this migration gives
-- Master System's own organization row a real slug of the same kind
-- - generated by Postgres itself right here, so the actual value is
-- never written into this file or any commit history.
-- ============================================================
