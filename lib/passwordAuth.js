...
const USERS_TABLE = () => encodeURIComponent(process.env.AIRTABLE_USERS_TABLE || "Users");
const BASE = () => process.env.AIRTABLE_BASE_ID;
const AUTH_HEADERS = () => ({ Authorization: `Bearer ${process.env.AIRTABLE_API_KEY}` });
...
export async function findUserByUsername(username) {
  const url = new URL(`https://api.airtable.com/v0/${BASE()}/${USERS_TABLE()}`);
  url.searchParams.set("filterByFormula", `{Username} = "${String(username).replace(/"/g, '\\"')}"`);
  url.searchParams.set("maxRecords", "1");
  const resp = await fetch(url.toString(), { headers: AUTH_HEADERS() });
...
export async function findUserByEmail(email) {
  ...
  url.searchParams.set("filterByFormula", `LOWER({Email}) = LOWER("${escaped}")`);
  url.searchParams.set("maxRecords", "1");
  const resp = await fetch(url.toString(), { headers: AUTH_HEADERS() });
  if (!resp.ok) return null;
  const data = await resp.json();
...
export async function findUserByResetToken(token) {
  const url = new URL(`https://api.airtable.com/v0/${BASE()}/${USERS_TABLE()}`);
  url.searchParams.set("filterByFormula", `{Reset Token} = "${String(token).replace(/"/g, '\\"')}"`);
  url.searchParams.set("maxRecords", "1");
  const resp = await fetch(url.toString(), { headers: AUTH_HEADERS() });
  if (!resp.ok) return null;
  const data = await resp.json();
...
export async function updateUserFields(recordId, fields) {
  const resp = await fetch(`https://api.airtable.com/v0/${BASE()}/${USERS_TABLE()}/${recordId}`, {
    method: "PATCH",
