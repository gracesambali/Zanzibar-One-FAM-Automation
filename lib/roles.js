...
export const ROLES = {
  ...
  // Confirmed directly: a genuinely new, real role, not routed to an
  // existing one - biomedical equipment work orders had no obvious
  // fit among Mechanical, Electrical, Admin, or Property Manager, and
  // routing them to any of those would mean the wrong person gets
  // notified for critical medical equipment. Same permission shape as
  // the other routed technical roles.
  ...
  // A genuinely narrow role, confirmed directly: this login never
  // reaches the main dashboard at all — it goes straight to its own
  // standalone portal, built specifically around scanning and
  // inventory. Most flags here stay false not because they're
  // untested but because they're simply never relevant to what this
  // role actually does — kept correct anyway for real defense in
  // depth, the same discipline used everywhere else in this file.
  pharmacy: {
    label: "Pharmacy",
    viewDashboard: false,
...
export const ROUTED_ROLES = ["electrical_engineer", "mechanical_engineer", "admin", "property_manager", "biomedical_technician"];
