...
//
// Business Owner deliberately does NOT get approveProcurement or
// fulfillProcurement — confirmed directly: Business Owner should be
// able to SEE procurement status and history on any work order, but
// not act on it. Approve/Reject stays with whichever of the four core
// roles the job is actually routed to; Fulfill stays exclusively with
// Procurement. System Admin keeps every permission active, including
// these two — the deliberate fallback role for when things aren't
// going well for everyone else and someone needs to be able to act
...
export const ROLES = {
  ...
  admin: {
    ...
    viewWorkOrders: true,
    requestProcurement: true,
    approveProcurement: true,      // scoped to jobs routed to Admin specifically — enforced in work-orders.js, same pattern as closure review
    fulfillProcurement: false,
    markReadyForReview: true,
    reviewWorkOrderClosure: true,   // one of the four routed roles that can sign off closure
    enterWorkOrderCost: false,
  ...
  property_manager: {
    ...
    viewWorkOrders: true,
    requestProcurement: true,
    approveProcurement: true,      // scoped to jobs routed to Property Manager specifically — enforced in work-orders.js
    fulfillProcurement: false,
    markReadyForReview: true,
    reviewWorkOrderClosure: true,
    enterWorkOrderCost: false,
  ...
  business_owner: {
    ...
    viewWorkOrders: true,
    requestProcurement: true,
    approveProcurement: false,     // view-only on procurement — confirmed: sees status/history, does not act
