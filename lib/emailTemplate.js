...
export function buildFriendlyEmailHtml({ f, urgency, daysUntil, existingWoId, fromName, technicalPersonnelName }) {
  const recipient = technicalPersonnelName || "Team";
  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; color: #111827;">
    ...
    <div style="text-align: center; font-size: 11px; color: #9ca3af; margin-top: 14px;">
      Sent automatically by ${fromName || "Facility Asset Management System"}
    </div>
  </div>`;
}
...
// Sensor threshold-breach alert — same visual style, for out-of-range
// temperature/humidity readings from the IoT sensor pipeline.
export function buildSensorAlertEmailHtml({ assetName, location, sensorType, value, unit, targetRange, technicalPersonnelName, fromName }) {
  const recipient = technicalPersonnelName || "Team";
  return `
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; color: #111827;">
    <div style="background:#dc2626; color:#fff; padding: 18px 22px; border-radius: 10px 10px 0 0;">
      <div style="font-size: 12px; opacity: 0.85; letter-spacing: 0.5px; text-transform: uppercase;">Sensor Alert — ${sensorType} Out of Range</div>
      <div style="font-size: 18px; font-weight: 700; margin-top: 4px;">${assetName}</div>
    </div>
    <div style="border: 1px solid #e5e7eb; border-top: none; padding: 20px 22px; border-radius: 0 0 10px 10px;">
      <p style="font-size: 14px; line-height: 1.6; margin-top: 0;">Dear ${recipient},</p>
      <p style="font-size: 14px; line-height: 1.6;">
        A sensor reading outside the target range has been detected for <strong>${assetName}</strong> at <strong>${location}</strong>.
      </p>
      <p style="font-size: 14px; line-height: 1.6;">
        <strong>Reading:</strong> ${value}${unit}<br>
