...
export async function insert(table, fields) {
  // Same reasoning as update() above - stamped at creation too, so a
  // brand-new work order's status_changed_at is never null while
  // waiting for its first real update.
  if (table === "work_orders") {
    if ("status" in fields && !("status_changed_at" in fields)) {
      fields = { ...fields, status_changed_at: new Date().toISOString() };
    }
    if ("procurement_status" in fields && !("procurement_status_changed_at" in fields)) {
      fields = { ...fields, procurement_status_changed_at: new Date().toISOString() };
    }
  }
...
export async function update(table, id, fields, organizationId) {
  // Confirmed directly: auto-stamps whenever a work order's status or
  // procurement_status actually changes, centralized here rather than
  // in every individual call site that changes these fields - missing
  // even one call site would silently break "how long has this been
  // sitting in its current state," which the Business Owner/System
  // Admin stalled-item view depends on being genuinely accurate.
  if (table === "work_orders") {
    if ("status" in fields && !("status_changed_at" in fields)) {
      fields = { ...fields, status_changed_at: new Date().toISOString() };
    }
