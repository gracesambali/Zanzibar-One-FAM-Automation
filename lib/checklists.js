...
// --- Tier 2: generic, discipline-level fallback checklists -----------
//
// Not sourced from a specific standard — deliberately generic. These
// exist purely to close the gap Grace identified: not every asset has
// a class in CHECKLISTS above, and staff-reported issues never have a
// specific asset at all, only a routed discipline. Every work order is
// now always routed to one of Mechanical / Electrical / Admin /
// Property Manager (or External) at creation, so keying the fallback
// to that guarantees it always resolves to something. Tier 1 above
// always wins whenever it actually matches — this only fills the gap.
const GENERIC_DISCIPLINE_CHECKLISTS = {
  "Electrical": {
    sourceStandard: null,
    items: [
      { task: "Isolate / de-energize before inspecting", frequency: "Every occurrence" },
      { task: "Verify de-energized with a tester", frequency: "Every occurrence" },
      { task: "Inspect for burn marks, loose connections, corrosion", frequency: "Every occurrence" },
      { task: "Carry out repair / replacement", frequency: "Every occurrence" },
      { task: "Restore power and test operation", frequency: "Every occurrence" },
      { task: "Confirm no abnormal heat, noise, or smell after restoring", frequency: "Every occurrence" },
    ],
  },
  "Mechanical": {
    sourceStandard: null,
    items: [
      { task: "Isolate the system / equipment", frequency: "Every occurrence" },
      { task: "Inspect for leaks, unusual noise, or wear", frequency: "Every occurrence" },
      { task: "Carry out repair / service", frequency: "Every occurrence" },
      { task: "Restart and observe for correct operation", frequency: "Every occurrence" },
      { task: "Check for leaks / vibration after restart", frequency: "Every occurrence" },
    ],
  },
  "Admin": {
    sourceStandard: null,
    items: [
      { task: "Assess extent of damage", frequency: "Every occurrence" },
      { task: "Confirm area is safe in the meantime", frequency: "Every occurrence" },
      { task: "Carry out repair or replacement", frequency: "Every occurrence" },
      { task: "Confirm area is clean and safe on completion", frequency: "Every occurrence" },
    ],
  },
  "Property Manager": {
    sourceStandard: null,
    items: [
      { task: "Confirm the issue directly with the tenant", frequency: "Every occurrence" },
      { task: "Determine if it's a facility issue or tenant-caused", frequency: "Every occurrence" },
      { task: "Resolve or escalate as appropriate", frequency: "Every occurrence" },
      { task: "Confirm resolution with the tenant", frequency: "Every occurrence" },
    ],
  },
};

// --- Tier 3: absolute last resort ------------------------------------
// Only reached if there's genuinely no routed discipline to key off —
// shouldn't happen in practice now that every work order is always
// routed, but this is the true floor: nothing ever renders with zero
// checklist. Just formalizes steps the work order already has fields
// for (Before/After Photo), rather than inventing new process.
const UNIVERSAL_FALLBACK_CHECKLIST = {
  sourceStandard: null,
  items: [
    { task: "Confirm the reported issue on-site", frequency: "Every occurrence" },
    { task: "Take a before photo", frequency: "Every occurrence" },
    { task: "Carry out the repair", frequency: "Every occurrence" },
    { task: "Take an after photo", frequency: "Every occurrence" },
    { task: "Retest / verify the fix", frequency: "Every occurrence" },
  ],
};

// Full tiered resolution — the function work orders and the asset
// register should actually call. Precise asset-class match first,
// generic discipline checklist second, universal fallback last. Never
// returns an empty item list, so a checklist section never has to
// silently render nothing.
export function getChecklistForWorkOrder(checklistClass, assignedRole) {
  if (checklistClass && CHECKLISTS[checklistClass]) {
    return { class: checklistClass, tier: 1, generic: false, ...CHECKLISTS[checklistClass] };
  }
  if (assignedRole && GENERIC_DISCIPLINE_CHECKLISTS[assignedRole]) {
    return { class: `${assignedRole} (General)`, tier: 2, generic: true, ...GENERIC_DISCIPLINE_CHECKLISTS[assignedRole] };
  }
  return { class: "General", tier: 3, generic: true, ...UNIVERSAL_FALLBACK_CHECKLIST };
}
