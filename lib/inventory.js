...
// Confirmed directly: every item is batch-tracked now, not decided
// category by category. Deciding this per-category (as it briefly
// was, limited to "Pharmaceutical") meant genuinely relevant cases
// outside that one category - a chemical, an adhesive, anything else
// with a real shelf life - never got tracked at all. Items with no
// real expiry just leave that field blank; FEFO naturally sorts a
// blank expiry to the back of priority, so this is harmless for
// anything that doesn't actually expire.
