...
export function createSessionToken(username, role) {
  ...
  const payload = JSON.stringify({ u: username, r: role, exp: Date.now() + SESSION_DURATION_MS });
  ...
  if (authHeader.startsWith("Bearer ")) {
    ...
    if (realKey && providedKey.length === realKey.length) {
      ...
      if (crypto.timingSafeEqual(providedBuf, realBuf)) {
        ...
        return { u: "api-integration", r: process.env.API_INTEGRATION_ROLE || "technician", isApiKey: true };
      }
    }
  }
