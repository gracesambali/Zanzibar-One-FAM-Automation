...
export function listRecords(table, { filterByFormula, sort, maxRecords, pageSize, offset, fields } = {}) {
  ...
  if (fields) fields.forEach((f, i) => { query[`fields[]`] = f; }); // Airtable expects repeated fields[] params; simple cases only
...
export function createRecord(table, fields) {
  return airtableRequest(encodeURIComponent(table), { method: "POST", body: { fields } });
}

export function updateRecord(table, recordId, fields) {
  return airtableRequest(`${encodeURIComponent(table)}/${recordId}`, { method: "PATCH", body: { fields } });
}
